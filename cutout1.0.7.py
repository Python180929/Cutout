...
class Cutout:
    ...
    def TwoImage(self, image):
        h, w = image.shape[:2]
        img1 = cv.resize(image, None, fx=572 / w, fy=711 / h)
        ...
        rect = (32, 32, 480, 711)
        cv.grabCut(img1, mask, rect, bgdModel, fgModel, 5, cv.GC_INIT_WITH_RECT)
        # 将图片的背景换位白色
        mask2 = np.where((mask == 2) | (mask == 0), 0, 1).astype('uint8')
        image2 = img1 * mask2[:, :, None]
        image2 += 255 * (1 - cv.cvtColor(mask2, cv.COLOR_GRAY2BGR))
        # print(image2.shape)
    ...
    def WorkOn(self):
        ...
        # param = "http://www.mobtop.com.cn/Uploads/letter/305/2018-12-04/5c064894e425a.jp
        # self.num = 5
        # self.ReadImage(param)
