...
class Cutout:
    ...
    def ReadImage(self, url):
        ...
        self.ImageCutting(image,url)

    # 将护照上的照片裁剪下来
    def ImageCutting(self, image, url):
        h, w = image.shape[:2]
        # print(h, w)
        # 将护照上右面有人体照片的部分裁掉
        ...
        self.ImgPreprocessing(img, url)

    # 图片处理
    def ImgPreprocessing(self, image, url):
        ...
        self.CutImage(image2, url)

    # 图片大小调整
    def CutImage(self,image, url):
        # 图片裁剪
        image = image[0:620, 57:527]
        # 图片缩放
        image = cv.resize(image, None, fx=354/470, fy=472/620)
        self.ImageDistinct(image, url)

    # 处理好的图片保存到Images文件夹
    def ImageDistinct(self, image, url):
        if not os.path.exists(self.path):
            os.makedirs(self.path)
        # print(url)
        label = self.path + '/' + url[-12:]
        cv.imwrite(label, image)
    ...
    def WorkOn(self):
        ...
        # param = "http://www.mobtop.com.cn/Uploads/letter/154/2018-05-04/5aec211f8dcae.jpeg"
        # param = "http://qiniuyun.mobtop.com.cn/jpg1548379410_73525907.jpg"
        # param = "http://www.mobtop.com.cn/Uploads/letter/305/2018-12-04/5c064894e425a.jpg"
        # self.ReadImage(param)
